...
export const BillingPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-red-950 to-slate-900 rounded-2xl border border-red-800/40 p-6 text-white shadow-xl relative overflow-hidden space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              <span className="text-xs uppercase tracking-wider font-semibold text-red-400">Current Plan</span>
              <span
          ...
          <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${
                (usage?.users || 0) >= (limits?.maxUsers || 1) ? 'bg-amber-500' : 'bg-red-700'
              }`}
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl border p-6 flex flex-col justify-between space-y-6 relative transition-all ${
                  isCurrent
                    ? 'border-red-700 ring-2 ring-red-700/20 shadow-md'
                    : 'border-slate-200 hover:border-slate-300'
                }`}
              >
                {p.popular && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-red-700 text-white text-[10px] uppercase font-extrabold px-3 py-0.5 rounded-full shadow-sm">
                    Most Popular
                  </span>
                )}
                  ...
                  {isCurrent ? (
                    ...
                    p.id === 'FREE' ? (
                      ...
                      <button
                        ...
                        disabled={isAnyPlanProcessing}
                        className="w-full bg-red-700 hover:bg-red-800 text-white text-xs font-bold py-2.5 rounded-xl transition-all shadow-sm disabled:opacity-50 flex items-center justify-center space-x-2"
                      >
